...
class SHA1 {
    ...
    */
    ...
    public function preProcess($message, $different_length=0){
        /*
        Pre-processing:
        ...
        */
            ...
            foreach (str_split(sprintf('%064b', $originalSize+$different_length), 8) as $bin) {
                $message .= chr(bindec($bin));
            }
            return $message;
    }
    
    public function preProcessKeyLength($message, $key_length){
        /*
        Pre-processing:
        append the bit '1' to the message i.e. by adding 0x80 if characters are 8 bits. 
        append 0 ≤ k < 512 bits '0', thus the resulting message length (in bits)
        ...
        */
            ...
            foreach (str_split(sprintf('%064b', $originalSize), 8) as $bin) {
                $message .= chr(bindec($bin));
                $padding .= chr(bindec($bin));
            }  
            
            return bin2hex($padding);
    }
    ...
    public function SHAfunction($step, $b, $c, $d)
    {
        switch ($step) {
            case 0;
                return ($b & $c) ^ (~$b & $d);
    ...
    public function hash_sha1($input, $h0=0x67452301, $h1=0xEFCDAB89, $h2=0x98BADCFE, $h3=0x10325476, $h4=0xC3D2E1F0) {
        /*$h0 = 0x67452301;
        $h1 = 0xEFCDAB89;
        $h2 = 0x98BADCFE;
        $h3 = 0x10325476;
        ...
        $message = $this->preProcess($input, $added_length);
